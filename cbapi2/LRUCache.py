...
class LRUCache(object):
    ...
    def getVersionDetails(cls):
        return "v %d.%d.%d (%s)" % (
        cls.getVersionMajor(), cls.getVersionMinor(), cls.getVersionRelease(), cls.getVersionTag())
    ...
    def __str__(self):
        ret = 'LRU Cache (cur=%d, max=%d (+%d),maxAge=%f ms)' % (
        self.size(), self.maxSize, self.elasticity, 0.001 * self.maxAge)
        return ret
...
class lrumemoized(object):
    ...
    def __call__(self, *args, **kwargs):
        ...
        # only cache the key and not the initial_information
        value = self.cache.get(args)
        if not value:
            value = self.func(*args, **kwargs)
            self.cache.put(args, value)
            return value
        else:
